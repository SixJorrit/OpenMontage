...
for p in paths:
    mt = mimetypes.guess_type(p)[0]; res = 'VIDEO' if mt.startswith('video') else ('IMAGE' if mt.startswith('image') else 'FILE')
...
for p, inp, t in zip(paths, inputs, d['stagedTargets']):
    ...
    if inp['resource'] in ('IMAGE','FILE'): fc['filename'] = inp['filename']   # video: filename is derived by Shopify
    files.append(fc)
r = gql('mutation($f:[FileCreateInput!]!){fileCreate(files:$f){files{id fileStatus ... on GenericFile{url}} userErrors{field message}}}', {'f': files})['fileCreate']
